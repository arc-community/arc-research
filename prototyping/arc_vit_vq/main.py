...
class RiddleLoader:
    ...
    def load_batch(
        ...
        inputs, tests, targets, target_boards = (x.to(device) for x in (inputs, tests, targets, target_boards))
        return inputs, tests, targets, target_boards, riddle_ids
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--enc_depth", default=16, type=int)
    parser.add_argument("--dec_depth", default=16, type=int)
    ...
    parser.add_argument("--project", default="arc_vit_vq", type=str, help="project name for wandb")
    parser.add_argument(
        "--name",
...
def eval_model(
    ...
    while remaining > 0:
        ...
        y = model(train_inputs, test_inputs)
...
def run_train(
    ...
    for step in range(1, max_steps + 1):
        ...
        y = model(train_batch, test_batch)
...
def main():
    ...
    max_training_riddles = args.max_training_riddles
    for fn in tqdm(file_names):
        ...
        if all(
            ...
            if max_training_riddles > 0 and len(training_set) > max_training_riddles:
                break
